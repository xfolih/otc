...
-- === Compatibility polyfill & helpers ===
-- Some OTC builds don't expose Tile:hasCreature(). Provide a safe fallback.
if Tile and not Tile.hasCreature then
  function Tile.hasCreature(self)
    if not self then return false end
    if self.getCreatures then
      local list = self:getCreatures()
      return type(list) == 'table' and #list > 0
    end
    if self.getTopCreature then
      return self:getTopCreature() ~= nil
    end
    return false
  end
end

local function hasCreatureOnTile(tile)
  return tile ~= nil and tile.hasCreature ~= nil and hasCreatureOnTile(tile) or false
end

local function firstCreatureOnTile(tile)
  if not tile then return nil end
  if tile.getCreatures then
    local list = tile:getCreatures()
    if type(list) == 'table' and list[1] then return list[1] end
  end
  if tile.getTopCreature then
    local c = tile:getTopCreature()
    if c and c.isMonster then return c end -- heuristic: Creature objects have isMonster()
  end
  return nil
end
-- === End polyfill ===
...
onTextMessage(function(mode, text)  
  ...
  if not tiles then return end -- säkerhetskontroll för tiles-arrayen  
  
  for i, tile in ipairs(tiles) do  
    -- Kontrollera att tile inte är nil innan vi anropar metoder på den  
    if not hasCreatureOnTile(tile) and tile:isWalkable() and #tile:getItems() > 9 then  
      local topThing = tile:getTopThing()  
      if not topThing then goto continue end -- hoppa över om inget topThing finns  
        
      if not isInPz() then  
        return useWith(3197, topThing) -- använd topThing variabeln istället för att anropa igen  
      else  
        if now < lastMoved + 200 then return end -- delay to prevent clogging  
        local nearTiles = getNearTiles(tile:getPosition())  
        if nearTiles then -- kontrollera att nearTiles inte är nil  
          for j, nearTile in ipairs(nearTiles) do -- använd j istället för i för att undvika variabelkollision  
            if nearTile then -- kontrollera att nearTile inte är nil  
              local tpos = nearTile:getPosition()  
              if tpos and pos() ~= tpos then -- kontrollera att tpos inte är nil  
...
local function breakFurniture(destPos)  
  ...
  if not tiles then return false end -- säkerhetskontroll för tiles-arrayen  
    
  for i, tile in ipairs(tiles) do  
    if not tile then goto continue end -- kontrollera att tile inte är nil  
    ...
    if topThing and (isWg or not table.find(furnitureIgnore, topThing:getId()) and topThing:isItem()) then  
      ...
      if not tpos then goto continue end -- kontrollera att position inte är nil  
...
local function pushPlayer(creature)  
  if not creature then return end -- kontrollera att creature inte är nil  
    
  local cpos = creature:getPosition()  
  if not cpos then return end -- kontrollera att position inte är nil  
  ...
  for i, tile in ipairs(tiles) do  
    ...
    if not pos then goto continue end -- kontrollera att position inte är nil  
...
CaveBot.registerAction("goto", "green", function(value, retries, prev)
  ...
  if not path2 then
    ...
    for i, dir in ipairs(path) do
      ...
      if tile then
          if hasCreatureOnTile(tile) then
              local creature = firstCreatureOnTile(tile)
