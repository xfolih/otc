...
-- === Clean Loot Formatter (console only) ===
local function formatLootMessage(text)
  -- Replace every occurrence of {id, {name}, #hex} with just the name (fallback to id->name via g_things)
  local changed = true
  while changed do
    local before = text
    text = text:gsub("{(%d+), {(.-)}, (#%x+)}", function(id, name, color)
      local itemId = tonumber(id)
      local thing = g_things.getThingType(itemId)
      local itemName = (name and name:match("%S")) and name or (thing and thing:getName() or ("item " .. id))
      return itemName
    end)
    changed = (text ~= before)
  end
  -- Optional: normalize VIP bonus spacing
  text = text:gsub("%s*%((vip bonus.-)%)", " (%1)")
  return text
end

onTextMessage(function(mode, text)
  if type(text) == "string" and text:lower():find("loot of") then
    local msg = formatLootMessage(text)
    displayTextMessage(20, msg) -- 20 = status (white) in client
    return true
  end
end)
-- === End Clean Loot Formatter ===
